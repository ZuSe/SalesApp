...
function iScroll (el, options) {
    ...
    window.addEventListener(START_EVENT, function(e){
        	if ( e.touches && e.touches.length > 1 ){
        		that.disableEventHandlers();
        		that.enabled = false
        	}else if ( ! that.enabled ){
        		that.enableEventHandlers();
       		    that.enabled = true;
        	}
        }, true );
    $(window).bind("scrollstop",function(){
    	...
    	//CR#690022-5,694238-1 when we start workflow from V-view and H-viwe, after pinch in/out several time and rotation several time, the save orientation screen's width appears different
    	//guess JQM somehow influence the values, couldn't find where/why. 
    	if ( that.lastWidth != window.innerWidth && ( window.innerWidth == 768 || window.innerWidth == 703 || window.innerWidth == 839 ))
    	{
    		onFrameResize();
    		that.lastWidth = window.innerWidth ;
    	}
...
iScroll.prototype = {
    ...
    lastScrolledPos:[],
    enabled: true,
    zoomState: {scale: 1, offsetLeft: 0, offsetTop: 0, startPos: {x0:0,y0:0,x1:0,y1:0}, endPos: {x0:0,y0:0,x1:0,y1:0}}, //offset to client area including padding
...
//################################# added by Yan #########################################//
    ...
    showPage:function( page, isDialog ){
    	var that = this;
    	var pageElement  =  isDialog ? page : document.getElementById( page );
    	var pageChildren  =  pageElement.children;
    	var contentPage = null;
    	var header = null;
    	var footer = null;
    	var listviewTitle = null;

        if (!hasjQueryMobile) {
            for( var i = 0 ; i < pageChildren.length; i++ ) {
                if ( pageChildren[i].getAttribute('data-role') == 'content'){
                    contentPage = pageChildren[i];
                }else if ( pageChildren[i].getAttribute('data-role') == 'header'){
                    header = pageChildren[i];
                } else  if ( pageChildren[i].getAttribute('data-role') == 'footer'){
                    footer = pageChildren[i];
                }else if (  pageChildren[i].getAttribute('class') != null &&
                        pageChildren[i].getAttribute('class').indexOf('listview_header') >= 0){
                    listviewTitle  = pageChildren[i];

                }
            }
        } else {
        	
        	//Check to see if we have ui-fixed-position, we need to move it out from scroller
        	var fixDiv  = $('div[data-role="custom-fix-position"]', pageElement);
        	var fixDivHeight = 0;
            ...
            {
                contentPage = contentNodes.get(0);
            }
            var headerNodes = $('div[data-role="header"]', pageElement);
            if (headerNodes.length > 0)
            {
                header = headerNodes.get(0);
            }
            var footerNodes = $('div[data-role="footer"]', pageElement);
            if (footerNodes.length > 0)
            {
                footer = footerNodes.get(0);
            }
            var listviewHeaderNodes = $('div[data-role="listview_header"]', pageElement);
      	...
      	that.element = contentPage.children[0];
    	...
    	var topPadding = $(contentPage).css('paddingTop');
    	var padding = 0;
    	if ( topPadding ) {
    	    padding = parseInt( topPadding.substring(0, topPadding.length -2 )) *2;
    	}
    	height = height - padding-7; //wrapper has 15px top and bottom paddings , added another 7 pix since footer bar was chopped just 7 pix in iphone simulator.
    
    	height = height - fixDivHeight;;
    	
    	if (!isNaN(height)) {
    		that.wrapper.style.height=height +'px';
    	}
    	...
    	//CR661978-1, commented following one line, since in some cases , it caused screen flash.
    	...
    	if (isTouch || that.options.desktopCompatibility) {
    		...
    		//that.element.addEventListener('gesturechange', that, false);
    		//that.element.addEventListener('gestureend', that, false);
    	}

    	if (that.options.checkDOMChanges &&
    			typeof(that.element.addEventListener) != "undefined") {
    		that.element.addEventListener('DOMSubtreeModified', that, false);
    	}

    	if (!isTouch && typeof(that.element.addEventListener) != "undefined") {
    		that.element.addEventListener('click', that, true);
    	}
   
    	var lastScrolledY = 0;
    	var lastObj = that.lastScrolledPos.pop();
    	if ( lastObj ) {
    		var secondLastObj = that.lastScrolledPos.pop();
    		if ( secondLastObj ) {
    			if ( secondLastObj.pageId === page ) {  //we are navigating back;
    				lastScrolledY = secondLastObj.y;
    			}else{
    				that.lastScrolledPos.push( secondLastObj); //It's not navigating back, add them back.
    				that.lastScrolledPos.push( lastObj);
    			}
    		}else{
    			that.lastScrolledPos.push( lastObj);
    		}
    	...
    	var y  =  parseInt( lastScrolledY);
    	
    	//CR661978-1,  setting postion causes screen flash.
    	if ( y !== 0 ) {
    	    that.setPosition(0,y , true);
    	}

 },

    handleTextAreaAutogrow:function(event ,autoGrowHeight ) {
       var that = iScroller;
       that.maxScrollY =   that.maxScrollY  - autoGrowHeight;
    },
    
    hidePage:function( page ) {
        var that = this;
        if ( that.element == null ) { /* TODO: why is null */
           return;
        }
        that.element.style = "";
        window.addEventListener('onorientationchange' in window ? 'orientationchange' : 'resize', that, false);
        ...
        if (isTouch || that.options.desktopCompatibility) {
            ...
            //that.element.removeEventListener('gesturestart', that, false);
            //that.element.removeEventListener('gesturechange', that, false);
            //that.element.removeEventListener('gestureend', that, false);
        }
        ...
        //remember the scrolled location for this page.
        that.lastScrolledPos.push( {pageId: page, y: this.y } );
    },
...
//########################################################################################//
	...
	// return true if this is a slider control (or a child of a slider)
	// Testing both 'role' and 'class' because depends on where you tap the control.
	// CR-682450
	isSliderControl: function (e) {
		var point = isTouch ? e.changedTouches[0] : e;
		var target = point.target;

		while (target.nodeType != 1) {
			target = target.parentNode;
		}
		target = target.parentNode;		// odd - but needed
		if ( (target.getAttribute('role') === 'slider') || (target.getAttribute('class') === 'slider') ) {
			return true;
		} else {
			return false;
		}
    ...
    //for native select control. 
    isSelectControl:function(e) {
    	var point = isTouch ? e.changedTouches[0] : e;
		var target = point.target;
    ...
    isTextControl:function(e) {
		...
		var isText = ( target.tagName === 'INPUT' && target.type === "text" )
				|| ( target.tagName === 'TEXTAREA' && target.type === "textarea") ;
		
		
		return  isIOS5() && isText
    },
    ...
    refresh: function () {
        ...
        this.maxScrollX = this.maxScroll().x;
        this.maxScrollY = this.maxScroll().y;
        that.directionX = 0;
        that.directionY = 0;
        if (that.scrollX) {
        ...
        that.scrollX = !that.options.bounceLock || that.scrollerWidth*that.zoomState.scale > that.scrollWidth;;
        that.scrollY = !that.options.bounceLock || that.scrollerHeight*that.zoomState.scale > that.scrollHeight;

        // Update horizontal scrollbar
        if (that.options.hScrollbar && that.scrollX && that.scrollerWidth > that.scrollWidth) {
    ...
    setPosition: function (x, y, hideScrollBars) {
        ...
        that.element.style.webkitTransform = translateOpen + that.x + 'px,' + that.y + 'px' + translateClose + ' scale(' + that.zoomState.scale + ')';
    ...
    touchStart: function(e) {
        ...
        if ( !that.isSliderControl(e) && ! that.isSelectControl(e ) && ! that.isTextControl( e)) {
            e.preventDefault();
            e.stopPropagation();
        }
    ...
    touchMove: function(e) {
        ...
        var tmpLeftOffset = that.options.scrollerPad - that.zoomState.offsetLeft;
        if (newX >= tmpLeftOffset || newX < that.maxScrollX) {
            newX = that.options.bounce ? Math.round(that.x + leftDelta / 3) : (newX >= tmpLeftOffset || that.maxScrollX>=0) ? tmpLeftOffset : that.maxScrollX;
        }
        var tmpTopOffset = that.options.menuHeightTop + that.options.scrollerPad - that.zoomState.offsetTop;
        if (newY >= tmpTopOffset || newY < that.maxScrollY) { 
            newY = that.options.bounce ? Math.round(that.y + topDelta / 3) : (newY >= tmpTopOffset || that.maxScrollY>=0) ? tmpTopOffset : that.maxScrollY;
        }
    ...
    touchEnd: function(e) {
        ...
        if (!that.moved) {
            ...
            if (isTouch) {
                ...
                var clickable = this.isClickable( target );
                if ( hasjQueryMobile && 
                	 !clickable &&
                     target.tagName != 'A' &&                        // ignore anchor
                     $(target).attr('class').indexOf('ui-btn')==-1 ) // ignore button
                {
            		var ve = createVirtualEvent($.event.fix(ev), "vclick");
            		$(target).trigger(ve);                    	
                }
                else
                {
        ...
        if (that.zoomState.scale == 1 && that.options.momentum) {
            momentumX = that.scrollX === true
                ? that.momentum(that.x - that.scrollStartX,
                                time,
                                that.options.bounce ? -that.x + that.scrollWidth/5 : -that.x,
                                that.options.bounce ? (that.maxScrollX < 0 ? that.x + that.scrollerWidth - that.scrollWidth : 0) + that.scrollWidth/5 : that.x + that.scrollerWidth - that.scrollWidth)
                : { dist: 0, time: 0 };
    ...
    transitionEnd: function () {
        ...
        if ( that.y == 0 && $( that.wrapper ).scrollTop() != 0 ) {
            $(that.wrapper).scrollTop( 0 );
        }
        if ( that.y == that.maxScrollY &&  $( that.wrapper ).scrollTop() != 0  ) {
           $(that.wrapper).scrollTop( 0 );
        }
    ...
    resetPosition: function () {
        ...
        var xOffset = -1*(that.zoomState.offsetLeft);
        if (that.x >= xOffset) {
            resetX = xOffset;
        } else if (that.x < that.maxScrollX) {
            resetX = that.maxScrollX;
        }
        var yOffset = -1*(that.zoomState.offsetTop);
        if (that.y >= yOffset) {
            resetY = yOffset;
        } else if (that.y < that.maxScrollY) {
            resetY = that.maxScrollY;
    ...
    offsetPos: function (obj){
    	return {x: $(obj).position().left, y: $(obj).position().top};
    },    
    
    maxScroll: function(){
    	// not zoom listers registered
        this.scrollWidth = this.wrapper.clientWidth;
        this.scrollHeight = this.wrapper.clientHeight;
        
        // not change after zoom
        this.scrollerWidth = this.element.scrollWidth;
        this.scrollerHeight = this.element.scrollHeight;

        // maximum scroll 
	    var xRight = (Math.round(this.scrollWidth - (this.scrollerWidth)*this.zoomState.scale))-this.zoomState.offsetLeft-this.options.scrollerPad*2;
	    var yBottom = (Math.round(this.scrollHeight - (this.scrollerHeight)*this.zoomState.scale))-this.zoomState.offsetTop;
	    //CR#694299-1
	    yBottom = yBottom - 35;
    ...
    zoomStart: function(e) {
    	...
    	document.addEventListener('webkitTransitionEnd', this, false);
    },
    ...
    zoomChange: function(e) {
    	...
    	var tmpScale = ((this.zoomState.scale)*(e.scale)).toFixed(1);
    	if (tmpScale > this.options.zoomMax || tmpScale < this.options.zoomMin) {e.preventDefault();e.stopPropagation();return;}
    	this.zoomState.scale = tmpScale;
    	
    	this.element.style.webkitTransform = ' scale(' + this.zoomState.scale  + ') ';
    	this.element.style.webkitTransitionDuration = '350ms';
    	this.zooming = true;
    },
    ...
    zoomStateChange:function(){
    	var diffX = this.options.scrollerPad;
    	var diffY = this.options.scrollerPad + this.options.menuHeightTop;
    	this.zoomState.offsetLeft = this.offsetPos(this.element).x - diffX;
    	this.zoomState.offsetTop = this.offsetPos(this.element).y - diffY;
    	this.x = this.zoomState.offsetLeft;
    	this.y = this.zoomState.offsetTop;
    	this.maxScrollX = this.maxScroll().x;
    	this.maxScrollY = this.maxScroll().y;
	},
	...
	isClickable: function ( target ) {
		if ( target && ( target.tagName === 'IMG' || target.tagName === 'SPAN' ||target.tagName.indexOf('H') >-1 )) {
		    var  tgParent = target.parentNode;
		    while( tgParent ) {
			    if ( tgParent.tagName === 'A' ){
			    	return true;
		    	}else{

...
$.widget( "mobile.dialog", $.mobile.widget, {
	...
	_create: function() {
		...
		headerCloseButton.bind( "vclick", function() {
			//CR#683498-2, commented it, custom dialog should handle in their code.
			//For selectmenu, handled in self.menuPageClose.click function.
			//following commented line would cause the page go to preview url
			//self.close();
		});
	...
	var extendSelect = function( widget ){
		...
		$.extend( widget, {
			...
			open: function() {
                ...
                var menuHeight = self.list.parent().outerHeight(),
				...
				if ( menuHeight > screenHeight - 80 || !$.support.scrollTop ) {
					...
					//<<---CR#684662-1 ,The last record could not display on dynamic choice . This only happened to iOS
					if ( isIOS() ){
					   var listView = self.menuPageContent.find( ".ui-selectmenu-list" );
				 	   if ( listView.length > 0 && listView.children().length > 0 ) {
						 var height = ( listView.children().length + 2 ) *  ( $(listView.children().get(0)).height() + 1 ) ; // 1px of  border top
						 listView.css('height', height +'px');
					  }
					}
					// CR684662-1  --->>
				} else {
